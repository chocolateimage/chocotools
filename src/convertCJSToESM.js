...
function replaceModuleExport(textEditor, editBuilder, exportName, objectName) {
    const replaces = {
        [`async function ${objectName}`]:
            exportName == null
                ? `export default async function ${objectName}`
    ...
    for (const [replaceFrom, replaceTo] of Object.entries(replaces)) {
        ...
        editBuilder.replace(
            new vscode.Range(
                textEditor.document.positionAt(index),
                textEditor.document.positionAt(index + replaceFrom.length)
            ),
            replaceTo
        );
        return null;
...
function commandConvertCJSToESM() {
    ...
    const indentCharacter = textEditor.options.insertSpaces
        ? " ".repeat(textEditor.options.indentSize)
        : "\t";
    ...
    textEditor.edit((editBuilder) => {
        ...
        if (exportsResult != null) {
            editBuilder.delete(
                new vscode.Range(
                    textEditor.document.positionAt(exportsResult.index - 1),
                    textEditor.document.positionAt(
                        exportsResult.index + exportsResult[0].length + 2
                    )
                )
            );
            for (const exportEntryRaw of exportsResult[1].split(",")) {
                ...
                failedExportTransformation.push(
                    replaceModuleExport(textEditor, editBuilder, exportName, objectName)
                );
            }
        ...
        for (let i = 0; i < textEditor.document.lineCount; i++) {
            ...
            failedExportTransformation.push(
                replaceModuleExport(textEditor, editBuilder, null, objectName)
            );
        ...
        for (const transformation of failedExportTransformation) {
            ...
            if (transformation.exportName == null) {
                ...
                if (transformation.exportName != transformation.objectName) {
                    extraExport.push(
                        `${transformation.objectName} as ${transformation.exportName}`
                    );
                } else {
        ...
        if (!didEdit) {
            vscode.window.showInformationMessage(
                "File " +
                    textEditor.document.fileName.replaceAll("\\", "/").split("/").pop() +
                    " is already in ESM format, nothing was done"
            );
        }
    });
}
